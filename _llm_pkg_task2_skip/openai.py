...
def _call_openai_responses(
    ...
    for m in messages:
        ...
        if role == "system":
            ...
            input_items.append({"type": "function_call_output", "call_id": m.get("tool_call_id", ""), "output": str(content)})

    body: dict = {"model": model_id, "input": input_items, "max_output_tokens": max_tokens}
    ...
    usage = resp.get("usage", {})
    return {"content": output_text, "tool_calls": tool_calls_out, "usage": {"input": usage.get("input_tokens", 0), "output": usage.get("output_tokens", 0)}}
