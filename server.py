...
class ModelRouter:
    ...
    def _pick_available(self, tier: int) -> str:
        ...
        for m in models:
            provider = m.split('/')[0]
            if provider == 'ollama':
                return m  # Ollama doesn't need API key
        ...
        for t in [2, 1, 3]:
            for m in self.TIERS.get(t, []):
                provider = m.split('/')[0]
                if provider == 'ollama' or vault.get(f'{provider}_api_key'):
                    return m
...
def execute_tool(name: str, args: dict) -> str:
    ...
    try:
        if name == 'exec':
            ...
            results = []
            # Search MEMORY.md, memory/*.md, and uploaded text files
            search_files = []
            if MEMORY_FILE.exists():
                search_files.append(('MEMORY.md', MEMORY_FILE))
            for f in sorted(MEMORY_DIR.glob('*.md')):
                search_files.append((f'memory/{f.name}', f))
            # Also search uploaded text files (RAG)
            uploads_dir = WORKSPACE_DIR / 'uploads'
            if uploads_dir.exists():
                for f in uploads_dir.glob('*'):
                    if f.suffix.lower() in ('.txt', '.md', '.py', '.js', '.json', '.csv', '.html', '.css', '.log', '.xml', '.yaml', '.yml'):
...
def build_system_prompt(full: bool = True) -> str:
    ...
    [삶앎 시스템]
    - 도구 사용 가능: exec(셸), read_file, write_file, edit_file, web_search, web_fetch, memory_read, memory_write, memory_search, image_generate, tts, usage_report, python_eval(Python실행), system_monitor(시스템감시), http_request(HTTP요청)
...
<div id="sidebar">
  ...
  <div class="side-nav">
    ...
    <div class="nav-section">도구 (15개)</div>
...
<div id="settings">
  <div class="settings-card">
    ...
    <select id="s-model" onchange="setModel(this.value)">
      ...
      <option value="ollama/llama3.3">🦙 Ollama Llama 3.3 (로컬)</option>
      <option value="ollama/qwen3">🦙 Ollama Qwen 3 (로컬)</option>
      <option value="ollama/gemma3">🦙 Ollama Gemma 3 (로컬)</option>
    </select>
