...
from .constants import VERSION, BASE_DIR, MEMORY_DIR, SOUL_FILE, VAULT_FILE, KST
...
def _get_agent_name() -> str:
    ...
    try:
        from .prompt import get_user_soul
        soul = get_user_soul()
        if soul:
...
class AgentExporter:
    ...
    def __init__(self, include_vault: bool = False,
    ...
    def export_agent(self) -> bytes:
        ...
        log.info(f"[EXPORT] Agent exported: {len(self._includes)} sections, "
                 f"{len(buf2.getvalue()) // 1024}KB")
    ...
    def _export_sessions(self, zf: zipfile.ZipFile):
        ...
        try:
            ...
            conn = _get_db()
            rows = conn.execute(
                'SELECT session_id, messages, updated_at FROM session_store'
            ).fetchall()
            for r in rows:
                ...
                zf.writestr(f'sessions/{sid}.json',
...
class AgentImporter:
        ...
        try:
            ...
            if Version(export_ver) > Version(VERSION):
                result.warnings.append(
                    f'Export version ({export_ver}) is newer than current ({VERSION})')
        except Exception:
    ...
    def _import_soul(self, zf: zipfile.ZipFile, result: ImportResult):
        try:
            ...
            from .prompt import set_user_soul
        ...
        try:
            ...
            for name in zf.namelist():
                ...
                existing = conn.execute(
                    'SELECT session_id FROM session_store WHERE session_id=?', (sid,)
                ).fetchone()
            ...
            for name in zf.namelist():
                ...
                if fname in _DATA_FILES:
                    ...
                    if dest.exists() and self.conflict_mode == 'merge':
                        ...
                        result.warnings.append(
                            f'{fname}: merge not supported for DB files, overwriting')
...
def quick_sync_export() -> dict:
    ...
    # SOUL.md
    try:
        from .prompt import get_user_soul, get_active_persona
    ...
    # Active persona
    try:
        from .prompt import get_active_persona
    ...
    # Failover config
    try:
        from .engine import get_failover_config
...
def quick_sync_import(data: dict):
    ...
    if 'routing' in data:
        try:
            from .engine import _save_routing_config
    ...
    if 'failover' in data:
        try:
            from .engine import save_failover_config
...
def export_agent(include_vault: bool = False,
                 include_sessions: bool = True,
...
def import_agent(zip_data: bytes,
                 conflict_mode: str = 'overwrite') -> ImportResult:
