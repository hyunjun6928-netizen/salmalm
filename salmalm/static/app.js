(function() {
  ...
  window.switchSession = function(sid) {
    ...
    var stored = localStorage.getItem(_storageKey$1(sid));
    if (stored) {
      ...
      if (hist.length) {
        ...
        hist.forEach(function(m) {
          if (m && m.role) addMsg(m.role, m.text, m.model);
        });
  ...
  window.exportChat = function(fmt) {
    var hist = JSON.parse(localStorage.getItem("salm_chat") || "[]");
    if (!hist.length) {
      addMsg("assistant", t("no-chat-export"));
      return;
    }
    var content = "";
    if (fmt === "json") {
  ...
  function addMsg$1(role, text, model) {
    if (text == null) text = "";
    ...
    if (_cleanText && _cleanText.length > 5) {
      var ttsBtn = document.createElement("button");
      ttsBtn.className = "tts-btn";
      ttsBtn.textContent = "🔊";
      ttsBtn.title = t("btn-tts-title");
    ...
    if (_cleanText && _cleanText.length > 5) {
      ...
      copyBtn.textContent = "📋";
      copyBtn.title = t("btn-copy-title") || "Copy";
    ...
    if (text) {
      ...
      regenBtn.title = t("btn-regen-title");
  ...
  window._cancelGeneration = function() {
    ...
    if (tr) tr.remove();
    addMsg$1("assistant", "⏹ " + (t("gen-cancelled") || "Generation cancelled."));
    btn.disabled = false;
    input.focus();
  ...
  async function doSend$1() {
    ...
    if (_inputText === "/branch") {
      ...
      fetch("/api/sessions/branch", {
        ...
        if (d.ok) {
          ...
          addMsg("assistant", t("branch-fail") + " " + (d.error || ""));
        }
      });
      return;
    }
    ...
    if (_filesToSend.length) {
      for (var fi = 0; fi < _filesToSend.length; fi++) {
        ...
        try {
          ...
          addMsg("assistant", t("upload-error") + " " + ue.message);
        }
      }
      window.clearFile();
    }
    ...
    var _safetyTimer = setTimeout(function() {
      ...
      if (tr) {
        tr.remove();
        addMsg("assistant", "⚠️ " + (t("timeout-msg") || "Response timed out. Please try again."));
      }
      btn.disabled = false;
    ...
    try {
      ...
      if (tr2) tr2.remove();
      addMsg("assistant", "❌ Error: " + se.message);
    } finally {
  ...
  function applyLang$1() {
    document.querySelectorAll("[data-i18n]").forEach(function(el) {
      ...
      if (el.tagName === "INPUT" || el.tagName === "TEXTAREA") el.placeholder = t$1(k);
      else el.textContent = t$1(k);
    });
    document.querySelectorAll("[data-i18n-ph]").forEach(function(el) {
      el.placeholder = t$1(el.getAttribute("data-i18n-ph"));
    });
    document.querySelectorAll("button").forEach(function(btn2) {
      var txt = btn2.textContent.trim();
      if (txt === "Save" || txt === "저장") btn2.textContent = t$1("btn-save");
      else if (txt === "Test" || txt === "테스트") btn2.textContent = t$1("btn-test");
    });
  ...
  window.setLang = function(v) {
    ...
    applyLang$1();
    if (typeof renderFeatures === "function") renderFeatures(document.getElementById("features-search") ? document.getElementById("features-search").value : "");
  };
  ...
  window.changePw = function() {
    ...
    if (!o || !n) {
      re.innerHTML = '<span style="color:#f87171">' + t("pw-enter-current") + "</span>";
      return;
    }
    if (n !== c) {
      re.innerHTML = '<span style="color:#f87171">' + t("pw-mismatch") + "</span>";
      return;
    }
    if (n.length < 4) {
      re.innerHTML = '<span style="color:#f87171">' + t("pw-min4") + "</span>";
      return;
    }
    fetch("/api/vault", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "change_password", old_password: o, new_password: n }) }).then(function(r) {
      ...
      if (d.ok) {
        re.innerHTML = '<span style="color:#4ade80">' + t("pw-changed") + "</span>";
        document.getElementById("pw-old").value = "";
        document.getElementById("pw-new").value = "";
        document.getElementById("pw-confirm").value = "";
      } else {
        re.innerHTML = '<span style="color:#f87171">' + t("pw-fail") + " " + (d.error || "") + "</span>";
      }
    }).catch(function(e) {
  ...
  window.removePw = function() {
    ...
    fetch("/api/vault", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "change_password", old_password: o, new_password: "" }) }).then(function(r) {
      ...
      if (d.ok) {
        re.innerHTML = '<span style="color:#4ade80">✅ ' + t("pw-remove") + "</span>";
        document.getElementById("pw-old").value = "";
  ...
  window.saveKey = function(vaultKey, inputId) {
    ...
    if (!v) {
      addMsg("assistant", "Please enter a key");
      return;
    }
    fetch("/api/vault", {
      method: "POST",
  ...
  window.googleConnect = function() {
    ...
    fetch("/api/vault", {
      ...
      if (!d.value) {
        re.innerHTML = '<span style="color:#f87171">' + t("google-no-client-id") + "</span>";
        return;
      }
  ...
  function _openSearchModal() {
    ...
    _searchInput.value = "";
    _searchResults.innerHTML = '<div style="padding:16px;text-align:center;color:var(--text2)">' + t("search-type-to-search") + "</div>";
    _searchInput.focus();
  }
  ...
  _searchModal.addEventListener("click", function(e) {
    if (e.target === _searchModal) _closeSearchModal$1();
  });
    ...
    _searchTimer = setTimeout(function() {
      fetch("/api/search?q=" + encodeURIComponent(q) + "&limit=15", { headers: { "X-Session-Token": _tok$1 } }).then(function(r) {
        ...
        if (!d.results || !d.results.length) {
          _searchResults.innerHTML = '<div style="padding:16px;text-align:center;color:var(--text2)">' + t("search-no-results") + ' "' + q + '"</div>';
          return;
        }
        var html = "";
        ...
        _searchResults.innerHTML = '<div style="padding:16px;text-align:center;color:var(--red)">' + t("search-error") + "</div>";
      });
    }, 300);
  });
      ...
      reader.onload = function(e) {
        try {
          ...
          fetch("/api/sessions/import", { method: "POST", headers: { "Content-Type": "application/json", "X-Session-Token": _tok$1 }, body: JSON.stringify({ messages: msgs, title }) }).then(function(r) {
            ...
            if (d.ok) {
              loadSessions();
              addMsg("assistant", "✅ " + (_lang === "ko" ? "대화를 가져왔습니다" : "Chat imported") + ": " + title);
            } else {
  ...
  _cmdPalette$1.innerHTML = '<input id="cmd-input" type="text" placeholder="' + t("cmd-placeholder") + '" autocomplete="off"><div id="cmd-results"></div>';
  ...
  _cmdOv.addEventListener("click", _closeCmdPalette$1);
  document.addEventListener("keydown", function(e) {
  ...
  document.addEventListener("click", function(e) {
    ...
    if (a === "newSession") window.newSession();
      ...
      if (!_qm) {
        if (window._msgQueue && window._msgQueue.length > 0) {
          if (confirm((t("queue-clear") || "Queue has ") + window._msgQueue.length + (t("queue-clear2") || " messages. Clear?"))) {
            window._msgQueue = [];
            var _qb0 = document.getElementById("queue-btn");
            if (_qb0) _qb0.textContent = "📥";
          }
        } else {
          window._toast && window._toast(t("queue-empty") || "Type a message first");
        }
        return;
      }
      ...
      fetch("/api/vault", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "set", key: "ollama_url", value: u }) }).then(function() {
        ...
        addMsg("assistant", "✅ Local LLM config saved");
        if (typeof window._loadModelRouter === "function") window._loadModelRouter();
      });
