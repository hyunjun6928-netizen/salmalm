import { _currentSession, _isAutoRouting, _sessionCache, _tok, btn, chat, costEl, fileIconEl, fileNameEl, filePrev, fileSizeEl, imgPrev, input, inputArea, modelBadge, pendingFile, pendingFiles, set_currentSession, set_isAutoRouting, set_pendingFile, set_pendingFiles, set_sessionCache, set_tok, settingsEl, t } from './globals';
  ...
  window.setFile=function(file){
    ...
    pendingFiles.push(file);set_pendingFile(pendingFiles[0]);
  ...
  window.clearFile=function(){set_pendingFile(null);set_pendingFiles([]);filePrev.style.display='none';imgPrev.style.display='none'};
