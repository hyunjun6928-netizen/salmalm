...
If the answer is insufficient, improve it now. If satisfactory, return it as-is."""
    ...
    def _get_injection_patterns(cls):
        if cls._INJECTION_PATTERNS is None:
            import re
            cls._INJECTION_PATTERNS = _re.compile(
    ...
    async def _execute_tools_parallel(self, tool_calls: list, on_tool=None, session=None) -> dict:
    ...
    async def _handle_tool_calls(self, result, session, provider, on_tool, on_status, consecutive_errors, recent_tool_calls):
        ...
        if valid_tools:
            exec_outputs = await self._execute_tools_parallel(valid_tools, on_tool, session)
