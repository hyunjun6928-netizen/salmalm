...
class Doctor:
    ...
    def check_config_integrity(self) -> dict:
        ...
        if not _SALMALM_DIR.exists():
            return _status(False, '~/.salmalm directory missing',
                           fixable=True, issue_id='missing_dir')
        ...
        if bad:
            return _status(False, f'Invalid config files: {", ".join(bad)}',
                           fixable=False, issue_id='bad_config')
    ...
    def check_session_integrity(self) -> dict:
        ...
        if bad:
            return _status(False, f'Bad sessions: {", ".join(bad)}',
                           fixable=True, issue_id='bad_sessions')
    ...
    def check_disk_space(self) -> dict:
        ...
        if free_gb < 1.0:
            return _status(False, f'Low disk: {free_gb:.1f}GB free',
                           fixable=False)
    ...
    def check_permissions(self) -> dict:
        ...
        if issues:
            return _status(False, f'Loose permissions: {", ".join(issues)}',
                           fixable=True, issue_id='permissions')
    ...
    def check_oauth_expiry(self) -> dict:
        ...
        try:
            ...
            if expiring:
                return _status(False, f'Tokens expiring soon: {", ".join(expiring)}',
                               fixable=False)
