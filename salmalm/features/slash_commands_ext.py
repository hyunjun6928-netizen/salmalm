...
from salmalm.core.cost import estimate_tokens, get_pricing as _get_pricing  # noqa: E402
...
def _cmd_usage(cmd: str, session, *, session_id="", **_) -> str:
    ...
    from salmalm.features.slash_commands import _get_session_usage
...
def _cmd_subagents(cmd: str, session, **_) -> str:
    ...
    if sub == "list":
        # Try subagent_manager first (used by spawn), fallback to SubAgent
        try:
            from salmalm.features.subagents import subagent_manager
            _uid_raw = getattr(session, "user_id", None)
            _uid = str(_uid_raw) if _uid_raw is not None else None
            _is_admin = getattr(session, "role", "") == "admin" or _uid is None  # single-user fallback
            tasks = subagent_manager.list_tasks(owner_uid=_uid, is_admin=_is_admin)
            if tasks:
                lines = ["🤖 **Sub-agents**\n"]
                for i, t in enumerate(tasks, 1):
                    status = t.get("status", "?")
                    icon = {"running": "🔄", "completed": "✅", "error": "❌", "failed": "❌", "stopped": "⏹", "pending": "⏳"}.get(status.split(".")[0], "❓")
                    tid = t.get("task_id", "?")
                    label = t.get("label") or t.get("description", "?")[:40]
                    runtime = f'{t.get("runtime_s", 0):.1f}s' if t.get("runtime_s") else ""
                    lines.append(f"{icon} #{i} `{tid}` — {label} [{status}] {runtime}")
                return "\n".join(lines)
        except Exception:
            pass
        ...
        # Try subagent_manager first
        try:
            from salmalm.features.subagents import subagent_manager
            _uid_raw = getattr(session, "user_id", None)
            _uid = str(_uid_raw) if _uid_raw is not None else None
            _is_admin = getattr(session, "role", "") == "admin" or _uid is None
            task = subagent_manager.get_task(arg, owner_uid=_uid, is_admin=_is_admin)
            if task:
                d = task.to_dict()
                status = d.get("status", "?")
                icon = {"running": "🔄", "completed": "✅", "error": "❌", "failed": "❌"}.get(status, "❓")
                result_text = d.get("result", "")[:500]
                return f"{icon} **Sub-agent {arg}**\nTask: {d.get('description', '')}\nStatus: {status}\nModel: {d.get('model', 'auto')}\n\n{result_text}"
        except Exception:
            pass
        ...
        results = subagent_manager.collect_results(parent_session=getattr(session, "id", "web"))
        if not results:
