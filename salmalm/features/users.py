...
class UserManager:
    ...
    def multi_tenant_enabled(self) -> bool:
        ...
        try:
            conn = sqlite3.connect(str(USERS_DB))
            row = conn.execute(
                "SELECT value FROM multi_tenant_config WHERE key='enabled'"
            ).fetchone()
            conn.close()
    ...
    def get_config(self, key: str, default: str = '') -> str:
        ...
        try:
            conn = sqlite3.connect(str(USERS_DB))
            row = conn.execute(
                "SELECT value FROM multi_tenant_config WHERE key=?", (key,)
            ).fetchone()
            conn.close()
    ...
    def set_config(self, key: str, value: str):
        ...
        conn = sqlite3.connect(str(USERS_DB))
        conn.execute(
            "INSERT OR REPLACE INTO multi_tenant_config (key, value) VALUES (?, ?)",
            (key, value))
        conn.commit()
        conn.close()
    ...
    def check_quota(self, user_id: int) -> dict:
        ...
        if not row:
            ...
            return {'ok': True, 'daily_remaining': self.DEFAULT_DAILY_LIMIT,
    ...
    def set_quota(self, user_id: int, daily_limit: Optional[float] = None,
        ...
        if monthly_limit is not None:
            conn.execute('UPDATE user_quotas SET monthly_limit=? WHERE user_id=?',
    ...
    def get_quota(self, user_id: int) -> dict:
        ...
        conn.close()
        if not row:
            return {'daily_limit': self.DEFAULT_DAILY_LIMIT,
                    'monthly_limit': self.DEFAULT_MONTHLY_LIMIT,
                    'current_daily': 0, 'current_monthly': 0}
        return {
            'daily_limit': row[0], 'monthly_limit': row[1],
            'current_daily': row[2], 'current_monthly': row[3],
    ...
    def _maybe_reset_quotas(self, user_id: int):
        ...
        conn = sqlite3.connect(str(USERS_DB))
        row = conn.execute(
            'SELECT last_daily_reset, last_monthly_reset FROM user_quotas WHERE user_id=?',
            (user_id,)
        ).fetchone()
        if not row:
            conn.close()
    ...
    def reset_all_daily_quotas(self):
        ...
        conn = sqlite3.connect(str(USERS_DB))
        count = conn.execute(
            'UPDATE user_quotas SET current_daily=0, last_daily_reset=?', (now,)
        ).rowcount
        conn.commit()
        conn.close()
        if count:
    ...
    def reset_all_monthly_quotas(self):
        ...
        conn = sqlite3.connect(str(USERS_DB))
        count = conn.execute(
            'UPDATE user_quotas SET current_monthly=0, last_monthly_reset=?', (now,)
        ).rowcount
        conn.commit()
        conn.close()
        if count:
    ...
    def get_user_settings(self, user_id: int) -> dict:
        ...
        if not row:
            return {
                'model_preference': 'auto', 'persona': 'default',
                'routing_config': {}, 'tts_enabled': False,
                'tts_voice': 'alloy', 'extra': {}
            }
        try:
    ...
    def set_user_settings(self, user_id: int, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if key in ('model_preference', 'persona', 'routing_config',
                       'tts_enabled', 'tts_voice', 'settings_json'):
                conn.execute(f'UPDATE user_settings SET {key}=? WHERE user_id=?',
                             (value, user_id))
        conn.commit()
        conn.close()
    ...
    def get_user_by_telegram(self, chat_id: str) -> Optional[dict]:
        ...
        return {
            ...
            'enabled': bool(row[3]), 'tg_username': row[4]
        }
    ...
    def register_telegram_user(self, chat_id: str, password: str,
                               tg_username: str = '') -> dict:
    ...
    def get_all_users_with_stats(self) -> List[dict]:
        ...
        for r in rows:
            ...
            session_count = conn.execute(
                'SELECT COUNT(*) FROM session_store WHERE user_id=?', (uid,)
            ).fetchone()[0] or 0

            # Get total cost from usage_stats
            cost_row = conn.execute(
                'SELECT SUM(cost) FROM usage_stats WHERE user_id=?', (uid,)
            ).fetchone()
            total_cost = cost_row[0] if cost_row and cost_row[0] else 0

            # Get telegram link
            tg_row = conn.execute(
                'SELECT chat_id FROM telegram_users WHERE user_id=?', (uid,)
            ).fetchone()

            user_data = {
                'id': uid, 'username': r[1], 'role': r[2],
            ...
            if quota_row:
                user_data['quota'] = {
                    'daily_limit': quota_row[0], 'monthly_limit': quota_row[1],
                    'current_daily': quota_row[2], 'current_monthly': quota_row[3],
                }
    ...
    def toggle_user(self, user_id: int, enabled: bool) -> bool:
        ...
        conn = sqlite3.connect(str(USERS_DB))
        cursor = conn.execute(
            'UPDATE users SET enabled=? WHERE id=? AND role != ?',
            (1 if enabled else 0, user_id, 'admin'))
        conn.commit()
    ...
    def get_user_by_username(self, username: str) -> Optional[dict]:
        ...
        conn = sqlite3.connect(str(USERS_DB))
        row = conn.execute(
            'SELECT id, username, role, enabled FROM users WHERE username=?', (username,)
        ).fetchone()
        conn.close()
        if not row:
            return None
