...
class Artifact:
    """Single artifact."""
    __slots__ = ('id', 'type', 'language', 'content', 'preview',
                 'session_id', 'created_at', 'metadata')

    def __init__(self, *, id: str, type: str, language: str = '',
                 content: str, session_id: str = '', created_at: float = 0,
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'content': self.content, 'preview': self.preview,
    ...
    def from_dict(cls, d: Dict) -> 'Artifact':
        return cls(id=d['id'], type=d['type'], language=d.get('language', ''),
                   content=d['content'], session_id=d.get('session_id', ''),
                   created_at=d.get('created_at', 0), metadata=d.get('metadata', {}))
...
class ArtifactManager:
    ...
    def _save(self):
        try:
            self._db_path.write_text(
                json.dumps([a.to_dict() for a in self._artifacts.values()],
    ...
    def add(self, type: str, content: str, language: str = '',
            session_id: str = '', metadata: Optional[Dict] = None) -> Artifact:
        """Add an artifact."""
        aid = self._make_id(content)
        a = Artifact(id=aid, type=type, language=language, content=content,
    ...
    def export_all(self) -> str:
        ...
        return json.dumps([a.to_dict() for a in self._artifacts.values()],
                          ensure_ascii=False, indent=2)
...
def register_tools(registry_module=None):
    ...
    try:
        ...
        register_dynamic('artifacts_list', lambda args: handle_artifacts_command('/artifacts list'), {
            'name': 'artifacts_list',
