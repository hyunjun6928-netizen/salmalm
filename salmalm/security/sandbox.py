...
def _bwrap_args(workspace: str, allow_network: bool = False) -> list:
    ...
    args = [
        ...
        '--ro-bind', '/bin', '/bin',
        '--ro-bind', '/lib', '/lib',
        '--ro-bind', '/lib64', '/lib64',  # may not exist
...
def _set_rlimits(timeout: int = 30, memory_mb: int = 512,
                 max_fds: int = 50, max_fsize_mb: int = 10):
    ...
    try:
        ...
        resource.setrlimit(resource.RLIMIT_FSIZE,
                           (max_fsize_mb * 1024 * 1024, max_fsize_mb * 1024 * 1024))
        try:
...
def sandbox_exec(command: str, timeout: int = 30, allow_network: bool = False,
                 memory_mb: int = 512, workspace: Optional[str] = None) -> dict:
    ...
    result = {
        ...
        'sandbox_level': caps['level'], 'method': 'none',
    }
...
class SandboxConfig:
    ...
    def __init__(self, timeout_s: int = 30, allow_network: bool = False,
                 max_memory_mb: int = 512, isolate_temp: bool = False):
...
def sandboxed_exec(command, config: Optional[SandboxConfig] = None,
    ...
    result = sandbox_exec(
        command, timeout=cfg.timeout_s,
        allow_network=cfg.allow_network, memory_mb=cfg.max_memory_mb,
    )
