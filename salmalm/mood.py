...
_KR_MOOD_KEYWORDS: Dict[str, List[str]] = {
    'happy': ['ㅋㅋ', 'ㅎㅎ', '^^', '기쁘', '좋아', '행복', '최고', '대박', '신나',
              '굿', '잘됐', '감사', '고마워', '사랑', '♡', '뿌듯', '기분 좋'],
    'sad': ['ㅠㅠ', 'ㅜㅜ', '슬프', '슬퍼', '우울', '그리워', '외로', '눈물', '아프',
            '힘들', '지치', '괴로', '속상', '서글', '안타까'],
    'angry': ['ㅡㅡ', '짜증', '화나', '열받', '미치', '빡치', '싫어', '짜증나', '화남',
              '분노', '어이없', '황당', '개빡'],
    'anxious': ['걱정', '불안', '초조', '떨리', '긴장', '무서', '두려', '어떡',
                '어쩌지', '어떻게', '망할', '큰일'],
    'excited': ['!!!', '와아', '대박', '미쳤', '오오', '헐', '우와', '캬',
                '신난다', '기대', '두근', '설레'],
    'tired': ['피곤', '졸려', '힘들', '지친', '녹초', '기력', '잠와', '쉬고싶',
              'zzz', '으으', '아 진짜'],
    'frustrated': ['안돼', '안되', '왜이러', '답답', '막힘', '모르겠', '이해안',
                   '포기', '못하겠', '도대체', '제발'],
}

# ── English emotion keywords ──
_EN_MOOD_KEYWORDS: Dict[str, List[str]] = {
    'happy': ['happy', 'great', 'awesome', 'love', 'wonderful', 'fantastic', 'joy',
              'amazing', 'excellent', 'perfect', 'glad', 'pleased', 'yay', 'woohoo'],
    'sad': ['sad', 'depressed', 'down', 'lonely', 'heartbroken', 'miss', 'cry',
            'tears', 'grief', 'sorrow', 'unhappy', 'miserable'],
    'angry': ['angry', 'furious', 'mad', 'hate', 'pissed', 'annoyed', 'rage',
              'outraged', 'infuriating', 'wtf', 'damn'],
    'anxious': ['anxious', 'worried', 'nervous', 'scared', 'afraid', 'panic',
                'stress', 'stressed', 'overwhelm', 'dread', 'fear'],
    'excited': ['excited', 'thrilled', 'pumped', 'stoked', 'omg', 'cant wait',
                'hyped', 'lets go', 'woo'],
    'tired': ['tired', 'exhausted', 'sleepy', 'drained', 'burned out', 'burnout',
              'fatigue', 'worn out', 'ugh'],
    'frustrated': ['frustrated', 'stuck', 'confused', "don't understand", "doesn't work",
                   'broken', 'why', 'impossible', 'give up', "can't figure"],
}
...
class MoodDetector:
    ...
    def _load_config(self) -> Dict[str, Any]:
        if MOOD_CONFIG_FILE.exists():
            try:
                return json.loads(MOOD_CONFIG_FILE.read_text(encoding='utf-8'))
            except (json.JSONDecodeError, OSError):
                pass
        return {'enabled': True, 'sensitivity': 'normal'}

    def _save_config(self):
        _ensure_dir()
        MOOD_CONFIG_FILE.write_text(
