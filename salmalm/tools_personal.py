...
from .constants import WORKSPACE_DIR, KST, BASE_DIR
...
def handle_note(args: dict) -> str:
    ...
    if action == 'save':
        ...
        with _db_lock:
            conn = _get_db()
            conn.execute('INSERT INTO notes (id, content, tags, created_at, updated_at) VALUES (?,?,?,?,?)',
...
_EXPENSE_CATEGORIES = {
    '식비': ['점심', '저녁', '아침', '식사', '밥', '커피', '음식', '카페', '치킨', '피자', '배달', 'food', 'lunch', 'dinner', 'coffee'],
...
def handle_expense(args: dict) -> str:
    ...
    if action == 'add':
        ...
        with _db_lock:
            conn = _get_db()
            conn.execute('INSERT INTO expenses (id, amount, category, description, date, created_at) VALUES (?,?,?,?,?,?)',
        ...
        with _db_lock:
            conn = _get_db()
            rows = conn.execute(
                "SELECT id, amount, category, description FROM expenses WHERE date = ? ORDER BY created_at",
                (today,)).fetchall()
            total = conn.execute("SELECT COALESCE(SUM(amount), 0) FROM expenses WHERE date = ?",
                                 (today,)).fetchone()[0]
            conn.close()
        if not rows:
        ...
        with _db_lock:
            ...
            rows = conn.execute(
                ...
                (f'{month}%',)).fetchall()
            grand_total = conn.execute("SELECT COALESCE(SUM(amount), 0) FROM expenses WHERE date LIKE ?",
                                       (f'{month}%',)).fetchone()[0]
            conn.close()
        if not rows:
...
def handle_save_link(args: dict) -> str:
    ...
    if action == 'save':
        ...
        with _db_lock:
            conn = _get_db()
            rows = conn.execute(
                "SELECT id, url, title, tags, saved_at FROM saved_links ORDER BY saved_at DESC LIMIT ?",
                (count,)).fetchall()
            conn.close()
        if not rows:
            return '🔖 저장된 링크가 없습니다.'
...
def _pomodoro_timer_func(session_id: str, duration_min: int, ptype: str):
    ...
    with _db_lock:
        conn = _get_db()
        conn.execute('UPDATE pomodoro_sessions SET ended_at = ?, completed = 1 WHERE id = ?',
                     (now, session_id))
        conn.commit()
        conn.close()
    ...
    # Send notification
    try:
        from .tools_misc import _send_notification_impl
        if ptype == 'focus':
    ...
    if action == 'start':
        ...
        with _pomodoro_lock:
            ...
            _pomodoro_state.update({
                'active': True, 'type': 'focus',
                'start_time': now.isoformat(), 'duration_minutes': duration,
                'session_id': sid,
            })
        with _db_lock:
            conn = _get_db()
            conn.execute('INSERT INTO pomodoro_sessions (id, started_at, type) VALUES (?,?,?)',
                         (sid, now.isoformat(), 'focus'))
            conn.commit()
            conn.close()
...
_DEFAULT_ROUTINES = {
    ...
    'evening': {
        ...
        'steps': [
            ...
            {'type': 'message', 'label': '📔 감사일기', 'content': '오늘 하루 감사한 일 3가지를 떠올려보세요:\n1. \n2. \n3. '},
