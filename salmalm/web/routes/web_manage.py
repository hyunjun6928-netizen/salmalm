...
class ManageMixin:
    ...
    def _post_api_backup_restore(self):
        ...
        # Zip bomb guard: check total decompressed size
        _MAX_DECOMPRESSED = 500 * 1024 * 1024  # 500 MB
        _total_size = sum(i.file_size for i in zf.infolist())
        if _total_size > _MAX_DECOMPRESSED:
            self._json({"ok": False, "error": f"Zip decompressed size too large ({_total_size} bytes)"}, 400)
            return

        # Safety: check for path traversal + symlink traversal
        import stat
        for info in zf.infolist():
            ...
            if name.startswith("/") or ".." in name:
                ...
                return
            # Reject symlinks (Unix external_attr top 16 bits = mode)
            if stat.S_ISLNK(info.external_attr >> 16):
                self._json({"ok": False, "error": f"ZIP contains symlink: {name} — rejected"}, 400)
                return
    ...
    def _post_api_vault(self):
        ...
        if action == "set":
            ...
            try:
                ...
                self._json({"error": "Vault operation failed"}, 500)
        elif action == "get":
            key = body.get("key")
            if not key:
                self._json({"error": "key required"}, 400)
    ...
    def _post_api_node_execute(self):
        """Post api node execute — requires admin auth + HMAC signature verification."""
        if not self._require_auth("admin"):
            return
        body = self._body

        # Verify HMAC signature from gateway dispatch
        import hmac as _hmac
        import hashlib as _hl
        sig_header = self.headers.get("X-Signature", "")
        if sig_header:
            from salmalm.features.nodes import gateway
            secret = getattr(gateway, "_gateway_token", "") or ""
            if secret:
        ...
        if not tool:
            ...
            return
        # Enforce tool-tier checks
        from salmalm.tools.tool_registry import is_tool_allowed_external
        if not is_tool_allowed_external(tool):
            self._json({"error": "Tool not allowed for external execution"}, 403)
            return
        try:
