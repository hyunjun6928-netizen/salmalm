...
class WebFilesMixin:
    ...
    def _post_api_agent_import_preview(self):
        ...
        try:
            ...
            manifest = _json.loads(zf.read("manifest.json")) if "manifest.json" in namelist else {}
            # Group files by top-level directory as sections
            sections: dict = {}
            for f in namelist:
                top = f.split("/")[0] if "/" in f else "__root__"
                sections.setdefault(top, []).append(f)
            # Normalised response matching frontend contract (99-migration.js:32-39)
            self._json({
                "ok": True,
                "manifest": manifest,
    ...
    def _post_api_agent_import(self):
        """Import a full agent ZIP archive (skills, memory, config).

        Validates every member path against the destination base to prevent
        path traversal (zip-slip), then delegates to migration.import_agent().
        """
        if not self._require_auth("user"):
            return
        import zipfile
        ...
        # ── Extract ZIP bytes and optional conflict_mode from multipart ──
        boundary = (
        ...
        for part in parts:
            if b"filename=" in part:
                ...
                if body_start > 0:
                    ...
                    if zip_data.endswith(b"\r\n"):
                        zip_data = zip_data[:-2]
            if b'name="conflict_mode"' in part or b"name='conflict_mode'" in part:
                body_start = part.find(b"\r\n\r\n")
                if body_start > 0:
                    val = part[body_start + 4:].strip()
                    if val.endswith(b"\r\n"):
                        val = val[:-2]
                    conflict_mode = val.decode(errors="replace").strip() or "overwrite"

        if not zip_data:
            self._json({"ok": False, "error": "No ZIP file found"}, 400)
            return

        # ── Path traversal validation (zip-slip prevention) ──
        try:
            zf = zipfile.ZipFile(_io.BytesIO(zip_data))
            for name in zf.namelist():
                p = Path(name)
                if p.is_absolute() or ".." in p.parts:
                    self._json({"ok": False, "error": f"Unsafe archive path: {name}"}, 400)
                    return
        except Exception as e:
            self._json({"ok": False, "error": f"Invalid ZIP: {e}"}, 400)
            return

        # ── Delegate to migration layer ──
        try:
            from salmalm.utils.migration import import_agent
            result = import_agent(zip_data, conflict_mode=conflict_mode)
            status = 200 if result.ok else 400
            self._json(result.to_dict(), status)
        except Exception as e:
            self._json({"ok": False, "error": "Internal server error"}, 500)

    def _post_api_upload(self):
        """Post api upload."""
        # Require authentication before accepting file data
        if not self._require_auth("user"):
            return
    ...
    def _get_api_sessions_export(self) -> None:
        """Handle GET /api/sessions/{id}/export (owner or admin only)."""
        ...
        if not m:
            ...
            return
        sid = re.sub(r'[^a-zA-Z0-9_\-:]', '_', m.group(1))
        uid = user.get("id")
        ...
        conn = _get_db()
        row = conn.execute(
            "SELECT messages, updated_at, user_id FROM session_store WHERE session_id=?",
            (sid,),
        ).fetchone()
        if not row:
            self._json({"error": "Session not found"}, 404)
            return
        if not is_admin and row[2] is not None and row[2] != uid:
    ...
    def _get_api_google_callback(self) -> None:
        ...
        try:
            ...
            scopes_raw = result.get("scope", "")
            scopes = _html_mod.escape(scopes_raw)  # V-2: prevent XSS from OAuth scope values
    ...
    def _get_uploads(self) -> None:
        ...
        # Uploads are loaded by <img>/<audio> tags which can't set headers.
        # Accept auth via: (1) normal headers, (2) ?token= query param, (3) cookie.
        import urllib.parse as _up
        _parsed = _up.urlparse(self.path)
        _qs = _up.parse_qs(_parsed.query)
        _qt = _qs.get("token", [""])[0]
        from salmalm.web.auth import extract_auth as _extract_auth, auth_manager
        user = _extract_auth(dict(self.headers))
        if not user and _qt:
            user = auth_manager.verify_token(_qt)
        if not user:
            # Cookie fallback
            cookie_hdr = self.headers.get("cookie", "")
            for part in cookie_hdr.split(";"):
                part = part.strip()
                if part.startswith("salm_token="):
                    _ct = part[len("salm_token="):]
                    user = auth_manager.verify_token(_ct)
                    if user:
                        break
        if not user:
